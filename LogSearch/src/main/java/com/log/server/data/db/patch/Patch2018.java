...
public class Patch2018 {
	...
	public void setDataSource(DataSource plutoDataSource) {
		Log.debug("creating data source assignement");
		this.jdbcTemplate = new JdbcTemplate(plutoDataSource);
	}
	...
	public void applyPatch(UserCredentialsModel user) throws Exception {
		...
		try {
			...
			Log.warn("Tried adding column 'visible' in table 'CREDENTIALS_ROLE', but it already exists.");
			columnExists = true;
		}
